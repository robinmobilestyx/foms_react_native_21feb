...
class PonSummons extends Component {
  constructor(props) {
    super(props);
  ...
  submit_addnotes_Button = async () => {
    if (this.state.results == '') {
      ...
      const abc = await new ReportsController().uploadNotes(
        addnotes,
        this.props,
        'PonSummons',
        Global_Attributes.RSSummonUploadNotes,
      );

      this.setState({Alert_Visibility: false, results: '', isLoading: false});
    }
  };
  ...
  Show_Cancel_Roadeside_Alert = (visible, ticket_no) => {
    this.setState({Cancel_Roadside: visible, ticket_no: ticket_no});
  };

  submit_cancelroadside_Button = async () => {
    if (this.state.remarks == '') {
      ...
      const abc = await new ReportsController().CancelRoadSide(
        addremark,
        this.props,
        'PonSummons',
        Global_Attributes.RSSummonCancelRoadSide,
      );
  ...
  Search_btn = async () => {
    ...
    const abc = await new ReportsController().getTicketDetails(
      ...
      this.props,
      'PonSummons',
      Global_Attributes.ponPart3Summons,
    );
    abc;
  ...
  render() {
    ...
    for (let i = 0; i < Length; i++) {
      ...
      console.log(ticketObj['pon_id' + i]);
        ...
        <View key={i} style={{flex: 1}}>
          ...
          <View style={{flexDirection: 'row', alignSelf: 'center'}}>
            <TouchableOpacity
              onPress={() => {
                this.props.navigation.navigate('Print', {
                  url: ticketObj['summon3_drivers' + i],
                });
              }}
            ...
            <TouchableOpacity
              onPress={() => {
                this.props.navigation.navigate('Print', {
                  url: ticketObj['summon3_officer' + i],
                });
              }}
              style={TicketsComponentStyle.offender_officer_court_button}>
              <Text
            ...
            <TouchableOpacity
              onPress={() => {
                this.props.navigation.navigate('Print', {
                  url: ticketObj['summon3_affidivat' + i],
                });
              }}
              style={TicketsComponentStyle.offender_officer_court_button}>
              <Text
